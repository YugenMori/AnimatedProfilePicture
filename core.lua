...
local function loadmodelPlayer()
    ...
    if ModelFrame_Model1 == nil then
        ...
        overlay:SetTexture("Interface\\Addons\\AnimatedProfilePicture\\tempback")
        ...
        local border = ModelFrame_Model1:CreateTexture(nil, "OVERLAY")
        border:SetTexture("Interface\\Addons\\AnimatedProfilePicture\\border")  -- Set this to your border texture
        border:SetAllPoints(ModelFrame_Model1)
        border:SetPoint("TOPLEFT", -2, 2)  -- Adjust these values to position the border correctly
        border:SetPoint("BOTTOMRIGHT", 2, -2)  -- Adjust these values to position the border correctly
        uiChanged = false
...
local function loadmodelTarget()
    ...
    if (GetWoWVersion > 100000) then
        ...
        bossrare:SetTexture("Interface\\Addons\\AnimatedProfilePicture\\UIUnitFrameBoss2x")
        bossrare:SetAllPoints(ModelFrameTarget)
    else
        ModelFrameTarget:SetAllPoints(TargetFramePortrait)
    end
    ModelFrameTarget:SetWidth(12)
    ModelFrameTarget:SetHeight(12)
    ...
    if ModelFrame_Model2 == nil then
        ...
        -- Create a border texture
        local border = ModelFrame_Model2:CreateTexture(nil, "OVERLAY")
        border:SetTexture("Interface\\Addons\\AnimatedProfilePicture\\border")  -- Set this to your border texture
        border:SetAllPoints(ModelFrame_Model2)
        border:SetPoint("TOPLEFT", -2, 2)  -- Adjust these values to position the border correctly
        border:SetPoint("BOTTOMRIGHT", 2, -2)  -- Adjust these values to position the border correctly
        uiChanged = false
    elseif uiChanged == true then
...
local TargetChanged = CreateFrame("Frame", nil, nil)
TargetChanged:RegisterEvent("PLAYER_TARGET_CHANGED")
TargetChanged:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_TARGET_CHANGED" then
